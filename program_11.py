...
if __name__ == '__main__':
    ...
    plt.plot(Wildcat_Clip['Discharge'], color='orangered',label='Wildcat')
    plt.xlabel('Date')
    ...
    # FIGURE3.4: Average annual monthly flow; using functions GetMonthlyStatistics and GetMonthlyAverages
    ...
    # FIGURE3.5:Return period of annual peak flow events
   ...
   # Reversing so that Rank 1 is the highest event, and rank N is the lowest event.
    ...
    plt.rcParams.update({'font.size':12})
    plt.legend(loc='best')
    plt.savefig('Exceedance Probability.png', dpi=96,bbox_inches='tight')   
    plt.show()
